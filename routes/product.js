...
router.get("/admin/:id", async (req,resp)=>{
    ...
    let categories_array =  data.categories
    let categories_string = "";
    categories_array.forEach((item)=>{
        categories_string = categories_string + item.trim();
        categories_string = categories_string + ",";
    })
    let categories = categories_string.substring(0,categories_string.length-1)
    resp.render("modifyProduct",{data:data, temp:categories,id:req.params.id})
